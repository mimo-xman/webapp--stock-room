...
| [`prompts/activity-book-etsy.md`](prompts/activity-book-etsy.md) | **Etsy activity book** — children's line-art activity pages (**at least 4 mixed activity types**: mazes, dot-to-dot, tracing, matching, counting…; every puzzle **solvable with ONE clear solution**, functional text correct) + cover + reference-based announcement images; STEP 4c QC loop; the agent researches demand and picks a NON-LIVING theme itself. | `[NUMBER OF ACTIVITY PAGES]` |
| [`prompts/party-invitations-etsy.md`](prompts/party-invitations-etsy.md) | **Etsy party invitations** — a cohesive set of fill-in 5×7 card designs (headline + labeled blank lines, **perfect spelling is a HARD RULE**) + reference-based announcement images (set fan, close-up, party scene); the agent researches the occasion + theme and picks them itself — NON-LIVING decorations only. | `[NUMBER OF INVITATION DESIGNS]` |
| [`prompts/wall-art-set-etsy.md`](prompts/wall-art-set-etsy.md) | **Etsy wall art set** — matching printable art prints (ONE palette, ONE ratio, ONE style; gallery quality, zero artifacts/banding; quote prints allowed if perfectly spelled) + reference-based announcement images (styled frames, gallery-wall mockups from the REAL prints); the agent researches style + theme and picks them itself — NON-LIVING. | `[NUMBER OF ART PRINTS]` |
| [`prompts/wall-art-set-etsy-local.md`](prompts/wall-art-set-etsy-local.md) | **Etsy wall art set — LOCAL engine** — the same wall art set mission, but instead of the hosted image API the agent CLONES the Zazo Image Studio repo, writes the owner's Cloudinary + MongoDB values into its `.env`, launches the server in its own environment (mandatory STEP 0: health check + smoke test) and drives the local API routes at `http://localhost` — every image must land in the owner's Cloudinary (its permanent URL is the registered `image_link`, never a localhost URL), every job in the owner's MongoDB. | `[NUMBER OF ART PRINTS]` + `[CLOUDINARY_CLOUD_NAME]`, `[CLOUDINARY_API_KEY]`, `[CLOUDINARY_API_SECRET]`, `[MONGODB_URI]`, `[MONGO_DB_NAME]` |
| [`prompts/printable-set-etsy.md`](prompts/printable-set-etsy.md) | **Etsy printable set** — a functional printable system (chore charts, planners, trackers, bingo, gift tags…): coordinated pages that are actually **usable** (aligned sections, real checkboxes, write-in space), text is the product (zero spelling errors) + reference-based announcement images; the agent researches the niche and designs the system itself — NON-LIVING accents only. | `[NUMBER OF PRINTABLE PAGES]` |
| [`prompts/clipart-bundle-etsy.md`](prompts/clipart-bundle-etsy.md) | **Etsy clipart bundle** — individual PNG elements **isolated on pure white** (one subject per image, closed silhouette — the owner removes backgrounds in post), ONE style + palette across the bundle, commercial use + reference-based sample-sheet announcement images; the agent researches theme + style and picks them itself — NON-LIVING. | `[NUMBER OF CLIPART ELEMENTS]` |
| [`prompts/digital-download-etsy.md`](prompts/digital-download-etsy.md) | **Etsy digital download (generic)** — the agent researches which digital product sells RIGHT NOW, picks the product AND its `product_type` itself (templates, games, educational resources, cards, seasonal sets — including any of the dedicated types above) and applies the matching type conventions; covers the `digital_download` and `other` filter values on the /etsy page. | `[NUMBER OF PRODUCT IMAGES]` |

All of them share the same six connection variables (the two API links + keys + backup repo
links) and the same file layout: a variables table, a ✂ **CUT HERE** line, and the prompt
below it. The **local-engine** variant (`wall-art-set-etsy-local`) swaps that wiring: no
hosted image API URL/key — the Zazo repo link becomes the clone source, and five `.env`
values (Cloudinary ×3 + MongoDB ×2) are provided; its `sharedVars` registry entry handles
the swap while reusing the same storage keys.
