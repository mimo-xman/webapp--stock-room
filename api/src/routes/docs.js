...
const endpointRows = [
  ...
  ['POST', '/api/images/bulk-used', 'WEBAPP MULTI-SELECTION — stamp many images in one request, on the platforms chosen in the popup. Body { used: true|false, platforms?: [adobe_stock, shutterstock, …], image_ids: [] } → { data: { marked, changed, images[], missing[] } } (changed = images whose flags actually flipped; already-marked targets are skipped). Upscales follow their original image automatically (no per-variant targets).', true],
  ['POST', '/api/images/bulk-fetch', 'WEBAPP MULTI-SELECTION — fresh-from-the-DB read of every selected image before acting (platform-picker stats, CSV build). Body { image_ids: [] } (max 200) → { data: { images[] (request order), missing[] } }', true],
