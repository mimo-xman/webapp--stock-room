# AGENT PROMPT — Stock Room — Etsy wall art set mission — LOCAL Zazo Image Studio engine
...
>
> **Variant of `prompts/wall-art-set-etsy.md` — same mission, ONE difference:** the agent
> does NOT call the hosted Zazo Image Studio API. It CLONES the Zazo Image Studio project
> itself, launches the server LOCALLY in its own environment, and uses the local API
> routes normally. You provide your Cloudinary + MongoDB values: every generated image is
> uploaded to YOUR Cloudinary (permanent public URL) and every job record is stored in
> YOUR MongoDB — nothing stays trapped in the agent's sandbox.
>
...
| `[ZAZO IMAGE STUDIO REPO LINK]` | The project the agent CLONES and RUNS LOCALLY — its image engine for this mission | `https://github.com/mimo-xman/webapp--zazo-image-studio` |
| `[CLOUDINARY_CLOUD_NAME]` | The Cloudinary cloud the local engine uploads every generated image to | `dnmre2q5g` |
| `[CLOUDINARY_API_KEY]` | Its API key | `123456789012345` |
| `[CLOUDINARY_API_SECRET]` | Its API secret | `f3x…` |
| `[MONGODB_URI]` | The MongoDB where the local engine stores its job records | `mongodb+srv://user:pass@cluster.mongodb.net` |
| `[MONGO_DB_NAME]` | Database name, SEPARATE from the URI | `zazo-image-studio` |
| `[STOCK ROOM API LINK]` | Base URL of the Stock Room (asset database) API — unchanged, still the hosted API | `https://stock-room-api.onrender.com` |
...
# Mission: create a wall art set (style + theme YOU research and pick, [NUMBER OF ART PRINTS] matching prints + its ANNOUNCEMENT images) and register it for Etsy — with a LOCAL image engine you launch yourself
...
**launch your own LOCAL instance of Zazo Image Studio (STEP 0)**, research Etsy's current
rules, research which wall art styles and themes are trending and most in demand RIGHT
NOW, **pick the style + theme yourself** (the owner does not choose them — you do, based
on your research), design the set, create one Stock Room session for it, generate every
art print, **and the set's ANNOUNCEMENT images (the marketing visuals that present the
set to buyers and convince them to buy)** with your local engine, visually verify each
image against the GLOBAL CONTENT RULES and the HARD RULES, register each compliant image
inside the session, then verify and report.

**Your image engine is LOCAL this time.** You do NOT call the hosted Zazo Image Studio
API: you CLONE the Zazo Image Studio project from [ZAZO IMAGE STUDIO REPO LINK], configure
its `.env` with the owner's Cloudinary and MongoDB values, install its dependencies,
start the server in your own environment, and call its API routes at
`http://localhost:<port>` exactly as you would call the hosted one — same routes, same
request bodies, same async job flow. The owner's values are what make the run durable:
every generated image is uploaded to the owner's **Cloudinary** (its permanent public URL
is the `image_link` you register in the Stock Room — a `localhost` URL dies with your
environment and is NEVER registered), and every job record is stored in the owner's
**MongoDB**. The Stock Room side is unchanged: the hosted API at [STOCK ROOM API LINK]
remains your asset database.
...
is generated FROM the real prints** (your local engine's reference-image mode — STEP 4b), so
...
## STEP 0 — Set up your LOCAL image engine (Zazo Image Studio) — FIRST, before anything else

This mission does NOT use the hosted image API. You run the Zazo Image Studio project
yourself, in your own environment, and you call its routes locally. Do this FIRST: if the
engine cannot be cloned, configured or started, the whole mission is impossible — report
that immediately instead of researching for nothing.

**0a — Clone the project.**

```
git clone [ZAZO IMAGE STUDIO REPO LINK]
```

Then read the project's README (and its `.env.example` if present) INSIDE the cloned repo
— they are your source of truth for the exact environment variable names, the install
command, the start command and the port. The variable names below are the usual ones; if
the README documents different ones, the README wins.

**0b — Configure the `.env` (the owner's storage accounts).** Create the `.env` file at
the repo root with the owner's values:
...
- **Cloudinary** is what makes every generated image durable: the engine uploads each
  result to the owner's Cloudinary and returns its permanent public URL
  (`result.cloudinaryUrl`) — that URL is the `image_link` you register in the Stock Room
  AND the reference URL you pass at STEP 4b. A generation whose image stays on the local
  disk is a dead image: it dies with your environment.
- **MongoDB** persists the engine's job records in the owner's database.
- **API key for the local routes:** the engine authenticates calls with the `X-API-Key`
  header. Find the API-key variable in the README/`.env.example`, set it to a strong value
  you generate yourself (e.g. `openssl rand -hex 24`), and remember it — every local call
  in this mission uses that key. If the README documents a different local auth (a fixed
  key, several accepted keys, no auth), follow the README.
- Fill every other REQUIRED variable the README/`.env.example` lists (default or sensible
  value) so the server starts cleanly.

**0c — Install, start, wait for readiness.**

```
npm install
npm start        # or the start command the README documents
```

- The startup output prints the port (usually settable via `PORT`). Your engine base URL
  is `http://localhost:<port>` — use it everywhere the hosted URL was used before.
- **Wait for readiness:** poll `GET http://localhost:<port>/health` (with your X-API-Key
  header) every 2–3 s until it answers 200 — only then start STEP 1. If the server exits
  or never becomes healthy, read its logs, fix the cause (missing env var, unreachable
  MongoDB, port conflict…), restart, poll again.
- Keep the server running for the WHOLE mission — generation, polling, downloads and
  image-to-image references all go through it. If it dies mid-run, restart it, wait for
  `/health` to answer 200, and resubmit the in-flight job.

**0d — Smoke-test the engine (MANDATORY, before any real work).** Submit ONE tiny test
generation — a trivially compliant prompt (e.g. `a simple flat terracotta circle on a
plain cream background, minimal flat vector`), quality `1K`, any ratio — and confirm:

1. the job reaches `succeeded`, and
2. `job.result.cloudinaryUrl` exists and starts with `https://res.cloudinary.com/`.

That proves the engine AND the owner's Cloudinary work end-to-end. If the Cloudinary
upload fails (no `cloudinaryUrl`), fix the three Cloudinary values in `.env` (0b), restart
the engine, and repeat the smoke test — do NOT generate a single real print before it
passes. The test image is throwaway: never register it anywhere.
...
## STEP 4 — Generate the images (your LOCAL Zazo Image Studio engine)
...
For each, use the **async** flow (do NOT use `wait: true`) — against YOUR local engine
(`http://localhost:<port>`, started at STEP 0):

### 4a — Art prints (text-to-image)

```
POST http://localhost:<port>/generate          ← your LOCAL engine (STEP 0c)
Headers: X-API-Key: <the key you configured in the engine's .env at STEP 0b>
Body: {
...
```
GET http://localhost:<port>/jobs/<jobId>
Headers: X-API-Key: <the key you configured in the engine's .env at STEP 0b>
...
- `succeeded` → take `job.result`:
  - `image_link` = `result.cloudinaryUrl` — REQUIRED in this variant: it is the permanent
    public URL of the image in the owner's Cloudinary, and the ONLY value you register in
    the Stock Room and pass as a reference at STEP 4b. A `localhost` file URL
    (`result.image.url`) dies with your environment — NEVER register it. If a succeeded
    job returns no `cloudinaryUrl`, treat it as a failed generation: fix the Cloudinary
    values in the engine's `.env` (STEP 0b), restart the engine, and resubmit the job.
- `failed` → read `job.error` (the engine already ran its own internal retries). Then apply the
...
The engine accepts `images` — an array of **1 to 5 reference images** — in the
...
`image_link` in the Stock Room at STEP 5): they are public `https://res.cloudinary.com/…`
URLs, and your local engine downloads them from the internet exactly like the hosted API
did. Example — the "what is inside" collage:
...
- the local engine: the repo you cloned (commit id), the port it runs on, the smoke-test
  result (STEP 0d), any mid-run restart, and confirmation that every registered
  `image_link` is a `https://res.cloudinary.com/…` URL (nothing left on localhost)
...
- **Local engine unreachable (connection refused / timeout)** → the server died: read its
  terminal output, fix the cause (missing env var, unreachable MongoDB, port conflict),
  restart it, wait for `GET /health` to answer 200, then resubmit the in-flight job. Keep
  it running until the final report is written.
- **Local engine `401`** → the `X-API-Key` you send does not match the key you wrote in the
  engine's `.env` at STEP 0b — align them. And remember each service has its own key: the
  Stock Room key is a DIFFERENT key (check you used the right key with the right API).
...
- **Engine crashes repeatedly** → you already have the repo cloned at STEP 0: re-read its
  README and `.env.example` (Node version, required variables, start command), fix the
  setup, restart.
- **Stock Room unusable after all of this** → clone the repo and read its README:
  `[STOCK ROOM REPO LINK]`.

---


## APPENDIX A — Your LOCAL Zazo Image Studio instance — API quick reference

Base URL: `http://localhost:<port>` — the port YOUR server printed at startup (STEP 0c).
Auth: header `X-API-Key: <the key you configured in the engine's .env at STEP 0b>`
(or query `?apiKey=` for file URLs). Same project, same routes, same request bodies as
the hosted API — running on your machine.
...
| `GET /health` | Service status (your readiness check — STEP 0c) |
...
Job result fields you care about: `result.cloudinaryUrl` (the permanent URL you register —
REQUIRED in this variant), `result.image.url`, `result.image.sha256`, `job.attempts[]`.
