...
13. **EVERY image is verified before it is saved — the STEP 4c check is not optional.** A
    ...
    regeneration loop (corrected prompt, max 3 generations per image, then redesign
    the scene) before
...
- `failed` → read `job.error`: the server already retried with fresh exit IPs. Then apply the
  ...
  - A failed generation is never a reason to deviate from the plan: every page of the plan is
    still generated — the page count still reaches [NUMBER OF COLORING PAGES].
...
3. **Generation 3 fails → STOP — a 4th generation is FORBIDDEN.** Keep the best of the 3
   ...
   the book (that one is never saved), and flag the image clearly in the final
